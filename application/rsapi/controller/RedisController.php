...
class RedisController extends Controller
{
     ...
     * @return bool
     */
    public function hSetEx($key, $expire){
    ...
    /**
     * 为hash表多个字段设置值，并设置过期时间
     * @param $key
     * @param $value
     * @param $expire
     * @return bool|int
     */
    public function hMsetEx($key, $value, $expire){
        if (!is_array($value))
            return 2;
    ...
    public function sAddEx($key, $value = 1, $expire = 1800){
    ...
    /**
     * 字符串
     * 设置数据缓存，不存在返回false，设置成功返回值，默认缓存半个小时
     * @param $key
     * @param $value
     * @param $expire
     */
    public function setexCache($key, $value, $expire = 1800){
        return $this->redis->setex($key, $expire, $value);
    }
    ...
    public function zLast($key){
        $result = $this->redis->zRange($key, -1, -1);
        return $result;
    }
     ...
     * APP 根据号码获取redis最新短信
     */
    public function getLatestSMS($phone_num, $num = 19)
    {
        $result = $this->redis->zRevRange($phone_num, 0, $num);
        $data = array();
    ...
    /**
     * 字符串 自定义
     * 键值不存在，新建字符串，存在，累加1，返回该键值
     * setex 返回为true
     * incr 返回为具体数值
     * 判断时判断是否为真即可
     */
    public function getStringIncExpire($key, $expire = 1800){
        $value = $this->redis->get($key);
        if (!$value){
            $result = $this->redis->setex($key, $expire, 1);
        }else{
            $result = $this->redis->incr($key);
        }
        return $result;
